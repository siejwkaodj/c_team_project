...
void flag_effect(){
    ...
    if(event_endtime[0]){
        // 보물 1초동안 보여주기
        printText("1초동안 보물의 위치가 드러납니다", 107, line);
        line += 2;
    }
    if(event_endtime[1]){
        // 교수 5초동안 숨기기
        printText("교수님이 휴가를 갑니다. 5초동안 교수님이 보이지 않습니다.", 107, line);
        line += 2;
    }
    if(event_endtime[2]){
        // 이동속도 두배
        printText("에너지드링크를 마셨습니다. 5초동안 이동 속도가 두배가 됩니다.", 107, line);
        line += 2;
    }
    if(event_endtime[3]){
        printText("", 107, line);
    ...
    if(event_endtime[5]){
        // 아무 효과도 x
        printText("찢어진 논문을 발견했습니다... 가슴이 아픕니다.", 107, line);
        line += 2;
    }
    if(event_endtime[6]){
        // 이동 방향 바뀜
    ...
    if(event_endtime[8]){
        // 깃발 개수 늘어나거나 보물 재배치
        printText("졸업논문 주제가 바뀌었습니다... 깃발 개수가 늘어나고 보물이 재배치됩니다.", 107, line);
        line += 2;
    }
    if(event_endtime[9]){
        // 깃발 숨기기
        printText("너무 오랫동안 밤을 샜습니다... 5초동안 깃발이 보이지 않습니다.",107, line);
        line += 2;
    }
    return;
}
